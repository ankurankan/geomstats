"""
Unit tests for special orthogonal group SO(n) on tensorflow.
"""
...
class TestSpecialOrthogonalGroupMethods(tf.test.TestCase):
    ...
    def setUp(self):
        ...
        with_angle_close_0 = 1e-10 * tf.convert_to_tensor([1., -1., 1.])
        with_angle_close_pi_low = ((gs.pi - 1e-9) / gs.sqrt(2.)
                                   * tf.convert_to_tensor([0., 1., -1.]))
        with_angle_pi = gs.pi / gs.sqrt(3.) * tf.convert_to_tensor([1., 1., -1.])
